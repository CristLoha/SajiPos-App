...
class LayananDialog extends StatelessWidget {
      ...
      child: Container(
        ...
        child: BlocBuilder<CostSettingBloc, CostSettingState>(
          builder: (context, costState) {
            ...
            final defaultFee = costSetting?.serviceFee ?? 0.0;

            return BlocBuilder<CartCubit, CartState>(
              builder: (context, cartState) {
                final currentFee = cartState.serviceFeeAmount;
                final isSelected = currentFee > 0;
                ...
                return Column(
                  ...
                  children: [
                    ...
                    if (defaultFee > 0)
                      _buildOption(
                        title: 'Biaya Layanan',
                        subtitle: formatCurrency.format(defaultFee),
                        isSelected: isSelected,
                        onChanged: () {
                          context.read<CartCubit>().updateCosts(
                            serviceFeeAmount: isSelected ? 0.0 : defaultFee,
                          );
                        },
                      )
                    else
                      const Center(
                        child: Text(
                          'Biaya layanan belum diatur di Pengaturan',
