...
class SyncCubit extends Cubit<SyncState> {
    ...
    if (lastSync != null) {
      ...
      emit(
        SyncState(
          isLoading: false,
          isSuccess: false,
          message: 'Terakhir sinkronisasi: $lastSync',
        ),
      );
    }
  }
  ...
  Future<void> syncData() async {
    ...
    result.fold(
      (failure) => emit(
        SyncState(isLoading: false, isSuccess: false, message: failure.message),
      ),
      (success) async {
        String msg = '';
        orderResult.fold((failure) => msg = '\n(Gagal kirim pesanan offline)', (
          syncedCount,
        ) {
          if (syncedCount > 0) {
        ...
        emit(
          SyncState(
            isLoading: false,
            isSuccess: true,
            message: 'Terakhir sinkronisasi: $now$msg',
          ),
        );
      },
    );
